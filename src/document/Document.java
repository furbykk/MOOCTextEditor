...
//		System.out.println("Pattern[" +pattern + "] Tokens: " + tokens + " size[" + tokens.size() + "]");
		return tokens;
	}
	...
	protected int countSyllables(String word) {
		...
		boolean singleChVowel = true;
		char[] chars = word.toCharArray();
		for(int i=0; i<chars.length;i++) {
			char currLowerChar = Character.toLowerCase(chars[i]);
			if(vowels.indexOf(currLowerChar)>=0) {
				if (i == chars.length - 1 && currLowerChar == 'e') {
					if(numSyllables==0){
						numSyllables ++;
						break;
					}
//					if(word.toLowerCase().indexOf("le")>0) {
//						numSyllables ++;
//						break;
//					}
					break;
...
//			numSyllables ++;
//		}
//		System.out.println("Number of syllables of " + word + " is " + numSyllables);
		return numSyllables;
	}
	...
	public static boolean testCase(Document doc, int syllables, int words, int sentences)
	{
		...
		if (wordsFound != words) {
			System.out.println("Incorrect number of words.  Found " + wordsFound
					+ ", expected " + words);
			passed = false;
		}
		if (sentFound != sentences) {
			System.out.println("Incorrect number of sentences.  Found " + sentFound
					+ ", expected " + sentences);
			passed = false;
		}
		if (syllFound != syllables) {
			System.out.println("Incorrect number of syllables.  Found " + syllFound
					+ ", expected " + syllables);
			passed = false;
		}
